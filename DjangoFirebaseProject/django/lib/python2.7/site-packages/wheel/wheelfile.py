...
class WheelFile(ZipFile):
    ...
    def __init__(self, file, mode='r'):
        ...
        if not basename.endswith('.whl') or self.parsed_filename is None:
            raise WheelError("Bad wheel filename ***REMOVED***!r***REMOVED***".format(basename))

        ZipFile.__init__(self, file, mode, compression=ZIP_DEFLATED, allowZip64=True)

        self.dist_info_path = '***REMOVED******REMOVED***.dist-info'.format(self.parsed_filename.group('namever'))
        self.record_path = self.dist_info_path + '/RECORD'
        self._file_hashes = OrderedDict()
        self._file_sizes = ***REMOVED******REMOVED***
        if mode == 'r':
            ...
            try:
                ...
                raise WheelError('Missing ***REMOVED******REMOVED*** file'.format(self.record_path))
                ...
                for line in record:
                    ...
                    if hash_sum:
                        ...
                        try:
                            ...
                            raise WheelError('Unsupported hash algorithm: ***REMOVED******REMOVED***'.format(algorithm))

                        if algorithm.lower() in ***REMOVED***'md5', 'sha1'***REMOVED***:
                            raise WheelError(
                                'Weak hash algorithm (***REMOVED******REMOVED***) is not permitted by PEP 427'
                                .format(algorithm))
    ...
    def open(self, name_or_info, mode="r", pwd=None):
        def _update_crc(newdata, eof=None):
            ...
            if eof and running_hash.digest() != expected_hash:
                raise WheelError("Hash mismatch for file '***REMOVED******REMOVED***'".format(native(ef_name)))
        ...
        if mode == 'r' and not ef_name.endswith('/'):
            if ef_name not in self._file_hashes:
                raise WheelError("No hash found for file '***REMOVED******REMOVED***'".format(native(ef_name)))
    ...
    def close(self):
        ...
        if self.fp is not None and self.mode == 'w' and self._file_hashes:
            content = '\n'.join('***REMOVED******REMOVED***,***REMOVED******REMOVED***=***REMOVED******REMOVED***,***REMOVED******REMOVED***'.format(fname, algorithm, hash_,
