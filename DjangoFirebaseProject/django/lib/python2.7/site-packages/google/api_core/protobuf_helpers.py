...
def from_any_pb(pb_type, any_pb):
    ...
    if not any_pb.Unpack(msg):
        raise TypeError(
            "Could not convert ***REMOVED******REMOVED*** to ***REMOVED******REMOVED***".format(
...
def check_oneof(**kwargs):
    ...
    if len(not_nones) > 1:
        raise ValueError(
            "Only one of ***REMOVED***fields***REMOVED*** should be set.".format(
...
def get(msg_or_dict, key, default=_SENTINEL):
    ...
    if isinstance(msg_or_dict, message.Message):
        ...
        raise TypeError(
            "get() expected a dict or protobuf message, got ***REMOVED***!r***REMOVED***.".format(
                type(msg_or_dict)
            )
        )
...
def set(msg_or_dict, key, value):
    ...
    if not isinstance(msg_or_dict, (collections_abc.MutableMapping, message.Message)):
        raise TypeError(
            "set() expected a dict or protobuf message, got ***REMOVED***!r***REMOVED***.".format(
                type(msg_or_dict)
            )
        )
    ...
    if subkey is not None:
        if isinstance(msg_or_dict, collections_abc.MutableMapping):
            msg_or_dict.setdefault(basekey, ***REMOVED******REMOVED***)
...
def field_mask(original, modified):
    ...
    if type(original) != type(modified):
        raise ValueError(
            ...
            'same type, received "***REMOVED***!r***REMOVED***" and "***REMOVED***!r***REMOVED***".'.format(
