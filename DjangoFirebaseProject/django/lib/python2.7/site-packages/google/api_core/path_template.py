...
# - "***REMOVED***name***REMOVED***": a single-segment wildcard named variable, for example
#   "books/***REMOVED***name***REMOVED***"
# - "***REMOVED***name=****REMOVED***: same as above.
# - "***REMOVED***name=*****REMOVED***": a multi-segment wildcard named variable, for example
#   "shelf/***REMOVED***name=*****REMOVED***"
# - "***REMOVED***name=/path/*/*****REMOVED***": a multi-segment named variable with a sub-template.
_VARIABLE_RE = re.compile(
    r"""
    (  # Capture the entire variable expression
        (?P<positional>\*\*?)  # Match & capture * and ** positional variables.
        |
        # Match & capture named variables ***REMOVED***name***REMOVED***
        ***REMOVED***
            (?P<name>[^/]+?)
...
def _expand_variable_match(positional_vars, named_vars, match):
    ...
    if name is not None:
        try:
            ...
            raise ValueError(
                "Named variable '***REMOVED******REMOVED***' not specified and needed by template "
                "`***REMOVED******REMOVED***` at position ***REMOVED******REMOVED***".format(name, match.string, match.start())
            )
        ...
        try:
            ...
            raise ValueError(
                ...
                "`***REMOVED******REMOVED***` at position ***REMOVED******REMOVED***".format(match.string, match.start())
            )
    else:
        raise ValueError("Unknown template expression ***REMOVED******REMOVED***".format(match.group(0)))
...
def expand(tmpl, *args, **kwargs):
    ...
    ..code-block:: python
        ...
        >>> expand('/v1/***REMOVED***name=shelves/*/books/****REMOVED***', name='shelves/1/books/3')
...
def validate(tmpl, path):
    ...
    .. code-block:: python
        ...
        False
        >>> validate('/v1/***REMOVED***name=shelves/*/books/****REMOVED***', /v1/shelves/1/books/3)
        True
        >>> validate('/v1/***REMOVED***name=shelves/*/books/****REMOVED***', /v1/shelves/1/tapes/3)
        False
