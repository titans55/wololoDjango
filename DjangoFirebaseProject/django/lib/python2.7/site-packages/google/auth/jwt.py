...
To encode a JWT use :func:`encode`::
    ...
    payload = ***REMOVED***'some': 'payload'***REMOVED***
...
def encode(signer, payload, header=None, key_id=None):
    ...
    header.update(***REMOVED***'typ': 'JWT', 'alg': 'RS256'***REMOVED***)
...
def _decode_jwt_segment(encoded_section):
    ...
    try:
        ...
        new_exc = ValueError('Can\'t parse segment: ***REMOVED***0***REMOVED***'.format(section_bytes))
...
def _unverified_decode(token):
    ...
    if token.count(b'.') != 2:
        raise ValueError(
            'Wrong number of segments in token: ***REMOVED***0***REMOVED***'.format(token))
...
def _verify_iat_and_exp(payload):
    ...
    for key in ('iat', 'exp'):
        if key not in payload:
            raise ValueError(
                'Token does not contain required claim ***REMOVED******REMOVED***'.format(key))
    ...
    if now < earliest:
        raise ValueError('Token used too early, ***REMOVED******REMOVED*** < ***REMOVED******REMOVED***'.format(now, iat))
    ...
    if latest < now:
        raise ValueError('Token expired, ***REMOVED******REMOVED*** < ***REMOVED******REMOVED***'.format(latest, now))
...
def decode(token, certs=None, verify=True, audience=None):
    ...
    if isinstance(certs, collections.Mapping):
        ...
        if key_id:
            if key_id not in certs:
                raise ValueError(
                    'Certificate for key id ***REMOVED******REMOVED*** not found.'.format(key_id))
    ...
    if audience is not None:
        ...
        if audience != claim_audience:
            raise ValueError(
                'Token has wrong audience ***REMOVED******REMOVED***, expected ***REMOVED******REMOVED***'.format(
...
class Credentials(google.auth.credentials.Signing,
    ...
    specify the JWT claims::
        ...
        credentials = jwt.Credentials.from_service_account_file(
            'service-account.json',
            audience=audience,
            additional_claims=***REMOVED***'meta': 'data'***REMOVED***)
    ...
    def __init__(self, signer, issuer, subject, audience,
        ...
        if additional_claims is None:
            additional_claims = ***REMOVED******REMOVED***
    ...
    def with_claims(self, issuer=None, subject=None, audience=None,
        ...
        new_additional_claims.update(additional_claims or ***REMOVED******REMOVED***)
