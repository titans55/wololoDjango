...
BAD_PATH_TEMPLATE = "A path element must be a string. Received ***REMOVED******REMOVED***, which is a ***REMOVED******REMOVED***."
...
BAD_REFERENCE_ERROR = (
    "Reference value ***REMOVED***!r***REMOVED*** in unexpected format, expected to be of the form "
    "``projects/***REMOVED******REMOVED***project***REMOVED******REMOVED***/databases/***REMOVED******REMOVED***database***REMOVED******REMOVED***/"
    "documents/***REMOVED******REMOVED***document_path***REMOVED******REMOVED***``."
)
WRONG_APP_REFERENCE = (
    "Document ***REMOVED***!r***REMOVED*** does not correspond to the same database " "(***REMOVED***!r***REMOVED***) as the client."
)
REQUEST_TIME_ENUM = enums.DocumentTransform.FieldTransform.ServerValue.REQUEST_TIME
_GRPC_ERROR_MAPPING = ***REMOVED***
...
def encode_dict(values_dict):
    ...
    Returns:
        ...
        values.
    """
    return ***REMOVED***key: encode_value(value) for key, value in six.iteritems(values_dict)***REMOVED***
...
def reference_value_to_document(reference_value, client):
    ...
    Raises:
        ValueError: If the ``reference_value`` is not of the expected
            format: ``projects/***REMOVED***project***REMOVED***/databases/***REMOVED***database***REMOVED***/documents/...``.
    ...
    # projects, ***REMOVED***project***REMOVED***, databases, ***REMOVED***database***REMOVED***, documents
...
def set_field_value(document_data, field_path, value):
    ...
    for element in field_path.parts[:-1]:
        current = current.setdefault(element, ***REMOVED******REMOVED***)
...
class DocumentExtractor(object):
    ...
    def __init__(self, document_data):
        ...
        self.array_removes = ***REMOVED******REMOVED***
        self.array_unions = ***REMOVED******REMOVED***
        self.set_fields = ***REMOVED******REMOVED***
...
class DocumentExtractorForMerge(DocumentExtractor):
    ...
    def _normalize_merge_paths(self, merge):
        ...
        for index in range(len(merge_paths) - 1):
            ...
            if lhs.eq_or_parent(rhs):
                raise ValueError("Merge paths overlap: ***REMOVED******REMOVED***, ***REMOVED******REMOVED***".format(lhs, rhs))
        ...
        for merge_path in merge_paths:
            ...
            try:
                ...
                raise ValueError("Invalid merge path: ***REMOVED******REMOVED***".format(merge_path))
    ...
    def _apply_merge_paths(self, merge):
        ...
        merged_set_fields = ***REMOVED******REMOVED***
        ...
        if unmerged_deleted_fields:
            raise ValueError(
                "Cannot delete unmerged fields: ***REMOVED******REMOVED***".format(unmerged_deleted_fields)
            )
        ...
        self.array_removes = ***REMOVED***
            path: values
            for path, values in self.array_removes.items()
            if path in merged_transform_paths
        ***REMOVED***

        self.array_unions = ***REMOVED***
            path: values
            for path, values in self.array_unions.items()
            if path in merged_transform_paths
        ***REMOVED***
    ...
    def __init__(self, document_data):
        ...
        for top_level_path in self.top_level_paths:
            for ancestor in top_level_path.lineage():
                if ancestor in tops:
                    raise ValueError(
                        "Conflicting field path: ***REMOVED******REMOVED***, ***REMOVED******REMOVED***".format(
        ...
        for field_path in self.deleted_fields:
            if field_path not in tops:
                raise ValueError(
                    "Cannot update with nest delete: ***REMOVED******REMOVED***".format(field_path)
                )
