...
_FIELD_PATH_MISSING_TOP = "***REMOVED***!r***REMOVED*** is not contained in the data"
_FIELD_PATH_MISSING_KEY = "***REMOVED***!r***REMOVED*** is not contained in the data for the key ***REMOVED***!r***REMOVED***"
_FIELD_PATH_WRONG_TYPE = (
    "The data at ***REMOVED***!r***REMOVED*** is not a dictionary, so it cannot contain the key ***REMOVED***!r***REMOVED***"
)
...
TOKENS_PATTERN = "|".join("(?P<***REMOVED******REMOVED***>***REMOVED******REMOVED***)".format(*pair) for pair in PATH_ELEMENT_TOKENS)
...
def _tokenize_field_path(path):
    ...
    if pos != len(path):
        raise ValueError("Path ***REMOVED******REMOVED*** not consumed, residue: ***REMOVED******REMOVED***".format(path, path[pos:]))
...
def split_field_path(path):
    ...
    for element in _tokenize_field_path(path):
        if want_dot:
            if element != ".":
                raise ValueError("Invalid path: ***REMOVED******REMOVED***".format(path))
            else:
                want_dot = False
        else:
            if element == ".":
...
class FieldPath(object):
        ...
        try:
            ...
            for element in elements:
                ...
                if _LEADING_ALPHA_INVALID.match(element):
                    raise ValueError(
                        "Non-alphanum char in element with leading alpha: ***REMOVED******REMOVED***".format(
                            element
                        )
                    )
    ...
    def __repr__(self):
        ...
        for part in self.parts:
            paths += "'" + part + "',"
        paths = paths[:-1]
        return "FieldPath(***REMOVED******REMOVED***)".format(paths)
    ...
    def lineage(self):
        ...
        return ***REMOVED***FieldPath(*self.parts[:index]) for index in indexes***REMOVED***
