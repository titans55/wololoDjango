...
def validate_uid(uid, required=False):
    ...
    if not isinstance(uid, six.string_types) or not uid or len(uid) > 128:
        raise ValueError(
            'Invalid uid: "***REMOVED***0***REMOVED***". The uid must be a non-empty string with no more than 128 '
...
def validate_email(email, required=False):
    ...
    if not isinstance(email, six.string_types) or not email:
        raise ValueError(
            'Invalid email: "***REMOVED***0***REMOVED***". Email must be a non-empty string.'.format(email))
    parts = email.split('@')
    if len(parts) != 2 or not parts[0] or not parts[1]:
        raise ValueError('Malformed email address string: "***REMOVED***0***REMOVED***".'.format(email))
    return email
...
def validate_phone(phone, required=False):
    ...
    if not isinstance(phone, six.string_types) or not phone:
        raise ValueError('Invalid phone number: "***REMOVED***0***REMOVED***". Phone number must be a non-empty '
                         'string.'.format(phone))
    if not phone.startswith('+') or not re.search('[a-zA-Z0-9]', phone):
        raise ValueError('Invalid phone number: "***REMOVED***0***REMOVED***". Phone number must be a valid, E.164 '
...
def validate_bytes(value, label, required=False):
    ...
    if not isinstance(value, six.binary_type) or not value:
        raise ValueError('***REMOVED***0***REMOVED*** must be a non-empty byte sequence.'.format(label))
    return value
...
def validate_display_name(display_name, required=False):
    ...
    if not isinstance(display_name, six.string_types) or not display_name:
        raise ValueError(
            'Invalid display name: "***REMOVED***0***REMOVED***". Display name must be a non-empty '
...
def validate_provider_id(provider_id, required=True):
    ...
    if not isinstance(provider_id, six.string_types) or not provider_id:
        raise ValueError(
            'Invalid provider ID: "***REMOVED***0***REMOVED***". Provider ID must be a non-empty '
...
def validate_photo_url(photo_url, required=False):
    ...
    if not isinstance(photo_url, six.string_types) or not photo_url:
        raise ValueError(
            'Invalid photo URL: "***REMOVED***0***REMOVED***". Photo URL must be a non-empty '
            'string.'.format(photo_url))
    try:
        parsed = urllib.parse.urlparse(photo_url)
        if not parsed.netloc:
            raise ValueError('Malformed photo URL: "***REMOVED***0***REMOVED***".'.format(photo_url))
        return photo_url
    except Exception:
        raise ValueError('Malformed photo URL: "***REMOVED***0***REMOVED***".'.format(photo_url))
...
def validate_timestamp(timestamp, label, required=False):
    ...
    try:
        ...
        raise ValueError('Invalid type for timestamp value: ***REMOVED***0***REMOVED***.'.format(timestamp))
    else:
        if timestamp_int != timestamp:
            raise ValueError('***REMOVED***0***REMOVED*** must be a numeric value and a whole number.'.format(label))
        if timestamp_int <= 0:
            raise ValueError('***REMOVED***0***REMOVED*** timestamp must be a positive interger.'.format(label))
...
def validate_int(value, label, low=None, high=None):
    ...
    if value is None or isinstance(value, bool):
        raise ValueError('Invalid type for integer value: ***REMOVED***0***REMOVED***.'.format(value))
    try:
        val_int = int(value)
    except TypeError:
        raise ValueError('Invalid type for integer value: ***REMOVED***0***REMOVED***.'.format(value))
        ...
        if low is not None and val_int < low:
            raise ValueError('***REMOVED***0***REMOVED*** must not be smaller than ***REMOVED***1***REMOVED***.'.format(label, low))
        if high is not None and val_int > high:
            raise ValueError('***REMOVED***0***REMOVED*** must not be larger than ***REMOVED***1***REMOVED***.'.format(label, high))
        return val_int
...
def validate_custom_claims(custom_claims, required=False):
    ...
    if len(claims_str) > MAX_CLAIMS_PAYLOAD_SIZE:
        raise ValueError(
            'Custom claims payload must not exceed ***REMOVED***0***REMOVED*** characters.'.format(
    ...
    if len(invalid_claims) > 1:
        ...
        raise ValueError('Claims "***REMOVED***0***REMOVED***" are reserved, and must not be set.'.format(joined))
    elif len(invalid_claims) == 1:
        raise ValueError(
            'Claim "***REMOVED***0***REMOVED***" is reserved, and must not be set.'.format(invalid_claims.pop()))
    return claims_str
