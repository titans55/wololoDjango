...
class _Validators(object):
    ...
    def check_string(cls, label, value, non_empty=False):
        ...
        if not isinstance(value, six.string_types):
            if non_empty:
                raise ValueError('***REMOVED***0***REMOVED*** must be a non-empty string.'.format(label))
            else:
                raise ValueError('***REMOVED***0***REMOVED*** must be a string.'.format(label))
    ...
    def check_number(cls, label, value):
        ...
        if not isinstance(value, numbers.Number):
            raise ValueError('***REMOVED***0***REMOVED*** must be a number.'.format(label))
        return value

    @classmethod
    def check_string_dict(cls, label, value):
        """Checks if the given value is a dictionary comprised only of string keys and values."""
        if value is None or value == ***REMOVED******REMOVED***:
            return None
        if not isinstance(value, dict):
            raise ValueError('***REMOVED***0***REMOVED*** must be a dictionary.'.format(label))
        non_str = [k for k in value if not isinstance(k, six.string_types)]
        if non_str:
            raise ValueError('***REMOVED***0***REMOVED*** must not contain non-string keys.'.format(label))
        non_str = [v for v in value.values() if not isinstance(v, six.string_types)]
        if non_str:
            raise ValueError('***REMOVED***0***REMOVED*** must not contain non-string values.'.format(label))
        return value
    ...
    def check_string_list(cls, label, value):
        ...
        if not isinstance(value, list):
            raise ValueError('***REMOVED***0***REMOVED*** must be a list of strings.'.format(label))
        non_str = [k for k in value if not isinstance(k, six.string_types)]
        if non_str:
            raise ValueError('***REMOVED***0***REMOVED*** must not contain non-string values.'.format(label))
        return value
...
class TopicManagementResponse(object):
    ...
    def __init__(self, resp):
        if not isinstance(resp, dict) or 'results' not in resp:
            raise ValueError('Unexpected topic management response: ***REMOVED***0***REMOVED***.'.format(resp))
...
class _MessageEncoder(json.JSONEncoder):
    ...
    def remove_null_values(cls, dict_value):
        return ***REMOVED***k: v for k, v in dict_value.items() if v not in [None, [], ***REMOVED******REMOVED***]***REMOVED***
    ...
    def encode_ttl(cls, ttl):
        ...
        if nanos:
            return '***REMOVED***0***REMOVED***.***REMOVED***1***REMOVED***s'.format(seconds, str(nanos).zfill(9))
        return '***REMOVED***0***REMOVED***s'.format(seconds)
    ...
    def encode_android_notification(cls, notification):
        ...
        ***REMOVED***
        ...
        if color and not re.match(r'^#[0-9a-fA-F]***REMOVED***6***REMOVED***$', color):
        ...
        ***REMOVED***
        ...
        if notification.custom_data is not None:
            ...
            for key, value in notification.custom_data.items():
                if key in result:
                    raise ValueError(
                        'Multiple specifications for ***REMOVED***0***REMOVED*** in WebpushNotification.'.format(key))
                result[key] = value
        return cls.remove_null_values(result)
        ...
        ***REMOVED***
        ...
        if aps.custom_data is not None:
            ...
            for key, val in aps.custom_data.items():
                ...
                if key in result:
                    raise ValueError('Multiple specifications for ***REMOVED***0***REMOVED*** in Aps.'.format(key))
                result[key] = val
        return cls.remove_null_values(result)
...
class _MessagingService(object):
    ...
    FCM_URL = 'https://fcm.googleapis.com/v1/projects/***REMOVED***0***REMOVED***/messages:send'
    IID_URL = 'https://iid.googleapis.com'
    IID_HEADERS = ***REMOVED***'access_token_auth': 'true'***REMOVED***
    JSON_ENCODER = _MessageEncoder()

    INTERNAL_ERROR = 'internal-error'
    UNKNOWN_ERROR = 'unknown-error'
    FCM_ERROR_CODES = ***REMOVED***
    ...
    ***REMOVED***
    ...
    def send(self, message, dry_run=False):
        data = ***REMOVED***'message': _MessagingService.encode_message(message)***REMOVED***
        if dry_run:
            data['validate_only'] = True
        try:
            headers = ***REMOVED***'X-GOOG-API-FORMAT-VERSION': '2'***REMOVED***
            ...
            if error.response is not None:
                ...
                msg = 'Failed to call messaging API: ***REMOVED***0***REMOVED***'.format(error)
    ...
    def make_topic_management_request(self, tokens, topic, operation):
        ...
        if not topic.startswith('/topics/'):
            topic = '/topics/***REMOVED***0***REMOVED***'.format(topic)
        data = ***REMOVED***
            'to': topic,
            'registration_tokens': tokens,
        ***REMOVED***
        url = '***REMOVED***0***REMOVED***/***REMOVED***1***REMOVED***'.format(_MessagingService.IID_URL, operation)
        try:
            resp = self._client.body(
                'post',
                url=url,
    ...
    def _handle_fcm_error(self, error):
        ...
        error_dict = data.get('error', ***REMOVED******REMOVED***)
        server_code = None
        ...
        if not msg:
            msg = 'Unexpected HTTP response with status: ***REMOVED***0***REMOVED***; body: ***REMOVED***1***REMOVED***'.format(
