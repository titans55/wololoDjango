...
class UserRecord(UserInfo):
    ...
    def __init__(self, data):
        ...
        if not isinstance(data, dict):
            raise ValueError('Invalid data argument: ***REMOVED***0***REMOVED***. Must be a dictionary.'.format(data))
...
class UserManager(object):
    ...
    def get_user(self, **kwargs):
        ...
        if 'uid' in kwargs:
            ...
            payload = ***REMOVED***'localId' : [_auth_utils.validate_uid(key, required=True)]***REMOVED***
        elif 'email' in kwargs:
            key, key_type = kwargs.pop('email'), 'email'
            payload = ***REMOVED***'email' : [_auth_utils.validate_email(key, required=True)]***REMOVED***
        elif 'phone_number' in kwargs:
            key, key_type = kwargs.pop('phone_number'), 'phone number'
            payload = ***REMOVED***'phoneNumber' : [_auth_utils.validate_phone(key, required=True)]***REMOVED***
        else:
            raise TypeError('Unsupported keyword arguments: ***REMOVED***0***REMOVED***.'.format(kwargs))

        try:
            response = self._client.request('post', 'getAccountInfo', json=payload)
        except requests.exceptions.RequestException as error:
            msg = 'Failed to get user by ***REMOVED***0***REMOVED***: ***REMOVED***1***REMOVED***.'.format(key_type, key)
            self._handle_http_error(INTERNAL_ERROR, msg, error)
        else:
            if not response or not response.get('users'):
                raise ApiCallError(
                    USER_NOT_FOUND_ERROR,
                    'No user record found for the provided ***REMOVED***0***REMOVED***: ***REMOVED***1***REMOVED***.'.format(key_type, key))
    ...
    def list_users(self, page_token=None, max_results=MAX_LIST_USERS_RESULTS):
        ...
        if not isinstance(max_results, int):
            ...
            raise ValueError(
                ...
                '***REMOVED***0***REMOVED***.'.format(MAX_LIST_USERS_RESULTS))

        payload = ***REMOVED***'maxResults': max_results***REMOVED***
        if page_token:
    ...
    def create_user(self, uid=None, display_name=None, email=None, phone_number=None,
        ...
        ***REMOVED***
        payload = ***REMOVED***k: v for k, v in payload.items() if v is not None***REMOVED***
        try:
        ...
        ***REMOVED***
        ...
        try:
            ...
            self._handle_http_error(
                USER_UPDATE_ERROR, 'Failed to update user: ***REMOVED***0***REMOVED***.'.format(uid), error)
        else:
            if not response or not response.get('localId'):
                raise ApiCallError(USER_UPDATE_ERROR, 'Failed to update user: ***REMOVED***0***REMOVED***.'.format(uid))
            return response.get('localId')
    ...
    def delete_user(self, uid):
        ...
        try:
            response = self._client.request('post', 'deleteAccount', json=***REMOVED***'localId' : uid***REMOVED***)
        except requests.exceptions.RequestException as error:
            self._handle_http_error(
                USER_DELETE_ERROR, 'Failed to delete user: ***REMOVED***0***REMOVED***.'.format(uid), error)
        else:
            if not response or not response.get('kind'):
                raise ApiCallError(USER_DELETE_ERROR, 'Failed to delete user: ***REMOVED***0***REMOVED***.'.format(uid))
    ...
    def import_users(self, users, hash_alg=None):
        ...
        try:
            if not users or len(users) > MAX_IMPORT_USERS_SIZE:
                raise ValueError(
                    'Users must be a non-empty list with no more than ***REMOVED***0***REMOVED*** elements.'.format(
        ...
        payload = ***REMOVED***'users': [u.to_dict() for u in users]***REMOVED***
