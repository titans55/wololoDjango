...
_USER_AGENT = 'Firebase/HTTP/***REMOVED***0***REMOVED***/***REMOVED***1***REMOVED***.***REMOVED***2***REMOVED***/AdminPython'.format(
...
def _parse_path(path):
    ...
    if not isinstance(path, six.string_types):
        raise ValueError('Invalid path: "***REMOVED***0***REMOVED***". Path must be a string.'.format(path))
    if any(ch in path for ch in _INVALID_PATH_CHARACTERS):
        raise ValueError(
            'Invalid path: "***REMOVED***0***REMOVED***". Path contains illegal characters.'.format(path))
...
class Reference(object):
    ...
    def child(self, path):
        ...
        if not path or not isinstance(path, six.string_types):
            raise ValueError(
                'Invalid path argument: "***REMOVED***0***REMOVED***". Path must be a non-empty string.'.format(path))
        if path.startswith('/'):
            raise ValueError(
                'Invalid path argument: "***REMOVED***0***REMOVED***". Child path must not start with "/"'.format(path))
    ...
    def get(self, etag=False, shallow=False):
        ...
        if etag:
            ...
            headers, data = self._client.headers_and_body(
                'get', self._add_suffix(), headers=***REMOVED***'X-Firebase-ETag' : 'true'***REMOVED***)
    ...
    def get_if_changed(self, etag):
        ...
        resp = self._client.request('get', self._add_suffix(), headers=***REMOVED***'if-none-match': etag***REMOVED***)
    ...
    def set_if_unchanged(self, expected_etag, value):
        ...
        try:
            headers = self._client.headers(
                'put', self._add_suffix(), json=value, headers=***REMOVED***'if-match': expected_etag***REMOVED***)
    ...
    def order_by_child(self, path):
        ...
        if path in _RESERVED_FILTERS:
            raise ValueError('Illegal child path: ***REMOVED***0***REMOVED***'.format(path))
...
class Query(object):
    ...
    def __init__(self, **kwargs):
        ...
        if order_by not in _RESERVED_FILTERS:
            if order_by.startswith('/'):
                raise ValueError('Invalid path argument: "***REMOVED***0***REMOVED***". Child path must not start '
        ...
        self._params = ***REMOVED***'orderBy' : json.dumps(order_by)***REMOVED***
        if kwargs:
            raise ValueError('Unexpected keyword arguments: ***REMOVED***0***REMOVED***'.format(kwargs))
    ...
    def _querystr(self):
        ...
        for key in sorted(self._params):
            params.append('***REMOVED***0***REMOVED***=***REMOVED***1***REMOVED***'.format(key, self._params[key]))
        return '&'.join(params)
...
class _Sorter(object):
    ...
    def __init__(self, results, order_by):
        if isinstance(results, dict):
            ...
            raise ValueError('Sorting not supported for "***REMOVED***0***REMOVED***" object.'.format(type(results)))
...
class _DatabaseService(object):
    ...
    def __init__(self, app):
        ...
        if auth_override != self._DEFAULT_AUTH_OVERRIDE and auth_override != ***REMOVED******REMOVED***:
            encoded = json.dumps(auth_override, separators=(',', ':'))
            self._auth_override = 'auth_variable_override=***REMOVED***0***REMOVED***'.format(encoded)
        else:
            self._auth_override = None
        self._timeout = app.options.get('httpTimeout')
        self._clients = ***REMOVED******REMOVED***
    ...
    def _validate_url(cls, url):
        ...
        if not url or not isinstance(url, six.string_types):
            raise ValueError(
                'Invalid database URL: "***REMOVED***0***REMOVED***". Database URL must be a non-empty '
                'URL string.'.format(url))
        parsed = urllib.parse.urlparse(url)
        if parsed.scheme != 'https':
            raise ValueError(
                'Invalid database URL: "***REMOVED***0***REMOVED***". Database URL must be an HTTPS URL.'.format(url))
        elif not parsed.netloc.endswith('.firebaseio.com'):
            raise ValueError(
                'Invalid database URL: "***REMOVED***0***REMOVED***". Database URL must be a valid URL to a '
                'Firebase Realtime Database instance.'.format(url))
        return 'https://***REMOVED***0***REMOVED***'.format(parsed.netloc)
    ...
    def _get_auth_override(cls, app):
        ...
        if not isinstance(auth_override, dict):
            raise ValueError('Invalid databaseAuthVariableOverride option: "***REMOVED***0***REMOVED***". Override '
...
class _Client(_http_client.JsonHttpClient):
    ...
    def __init__(self, credential, base_url, auth_override, timeout):
        ...
        _http_client.JsonHttpClient.__init__(
            ...
            headers=***REMOVED***'User-Agent': _USER_AGENT, 'X-Firebase-Decoding': '1'***REMOVED***)
    ...
    def request(self, method, url, **kwargs):
        ...
        if self.auth_override:
            ...
            if params:
                params += '&***REMOVED***0***REMOVED***'.format(self.auth_override)
            else:
    ...
    def extract_error_message(cls, error):
        ...
        try:
            ...
            if isinstance(data, dict):
                return '***REMOVED***0***REMOVED***\nReason: ***REMOVED***1***REMOVED***'.format(error, data.get('error', 'unknown'))
        except ValueError:
            pass
        return '***REMOVED***0***REMOVED***\nReason: ***REMOVED***1***REMOVED***'.format(error, error.response.content.decode())
