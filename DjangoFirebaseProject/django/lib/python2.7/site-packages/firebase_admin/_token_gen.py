...
class TokenGenerator(object):
    ...
    def _init_signing_provider(self):
        ...
        resp = self.request(url=METADATA_SERVICE_URL, headers=***REMOVED***'Metadata-Flavor': 'Google'***REMOVED***)
        if resp.status != 200:
            raise ValueError(
                'Failed to contact the local metadata service: ***REMOVED***0***REMOVED***.'.format(resp.data.decode()))
    ...
    def signing_provider(self):
        ...
        if not self._signing_provider:
            try:
                ...
                raise ValueError(
                    'Failed to determine service account: ***REMOVED***0***REMOVED***. Make sure to initialize the SDK '
                    'with service account credentials or specify a service account ID with '
                    'iam.serviceAccounts.signBlob permission. Please refer to ***REMOVED***1***REMOVED*** for more '
    ...
    def create_custom_token(self, uid, developer_claims=None):
        ...
        if developer_claims is not None:
            ...
            if disallowed_keys:
                if len(disallowed_keys) > 1:
                    error_message = ('Developer claims ***REMOVED***0***REMOVED*** are reserved and '
                                     'cannot be specified.'.format(
                                         ', '.join(disallowed_keys)))
                else:
                    error_message = ('Developer claim ***REMOVED***0***REMOVED*** is reserved and '
                                     'cannot be specified.'.format(
                                         ', '.join(disallowed_keys)))
        ...
        ***REMOVED***
        ...
        try:
            ...
            msg = 'Failed to sign custom token. ***REMOVED***0***REMOVED***'.format(error)
    ...
    def create_session_cookie(self, id_token, expires_in):
        ...
        if not isinstance(id_token, six.text_type) or not id_token:
            raise ValueError(
                'Illegal ID token provided: ***REMOVED***0***REMOVED***. ID token must be a non-empty '
        ...
        if isinstance(expires_in, bool) or not isinstance(expires_in, int):
            raise ValueError('Illegal expiry duration: ***REMOVED***0***REMOVED***.'.format(expires_in))
        if expires_in < MIN_SESSION_COOKIE_DURATION_SECONDS:
            raise ValueError('Illegal expiry duration: ***REMOVED***0***REMOVED***. Duration must be at least ***REMOVED***1***REMOVED*** '
                             'seconds.'.format(expires_in, MIN_SESSION_COOKIE_DURATION_SECONDS))
        if expires_in > MAX_SESSION_COOKIE_DURATION_SECONDS:
            raise ValueError('Illegal expiry duration: ***REMOVED***0***REMOVED***. Duration must be at most ***REMOVED***1***REMOVED*** '
    ...
    def _handle_http_error(self, code, msg, error):
        if error.response is not None:
            msg += '\nServer response: ***REMOVED***0***REMOVED***'.format(error.response.content.decode())
        else:
            msg += '\nReason: ***REMOVED***0***REMOVED***'.format(error)
...
class _JWTVerifier(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.short_name[0].lower() in 'aeiou':
            self.articled_short_name = 'an ***REMOVED***0***REMOVED***'.format(self.short_name)
        else:
            self.articled_short_name = 'a ***REMOVED***0***REMOVED***'.format(self.short_name)
    ...
    def verify(self, token, request):
        ...
        if not isinstance(token, six.binary_type) or not token:
            raise ValueError(
                'Illegal ***REMOVED***0***REMOVED*** provided: ***REMOVED***1***REMOVED***. ***REMOVED***0***REMOVED*** must be a non-empty '
                'string.'.format(self.short_name, token))

        if not self.project_id:
            raise ValueError(
                'Failed to ascertain project ID from the credential or the environment. Project '
                'ID is required to call ***REMOVED***0***REMOVED***. Initialize the app with a credentials.Certificate '
        ...
        project_id_match_msg = (
            'Make sure the ***REMOVED***0***REMOVED*** comes from the same Firebase project as the service account used '
            'to authenticate this SDK.'.format(self.short_name))
        verify_id_token_msg = (
            'See ***REMOVED***0***REMOVED*** for details on how to retrieve ***REMOVED***1***REMOVED***.'.format(self.url, self.short_name))

        error_message = None
        if not header.get('kid'):
            if audience == FIREBASE_AUDIENCE:
                error_message = (
                    '***REMOVED***0***REMOVED*** expects ***REMOVED***1***REMOVED***, but was given a custom '
                    'token.'.format(self.operation, self.articled_short_name))
            elif header.get('alg') == 'HS256' and payload.get(
                    'v') is 0 and 'uid' in payload.get('d', ***REMOVED******REMOVED***):
                error_message = (
                    '***REMOVED***0***REMOVED*** expects ***REMOVED***1***REMOVED***, but was given a legacy custom '
                    'token.'.format(self.operation, self.articled_short_name))
            else:
                error_message = 'Firebase ***REMOVED***0***REMOVED*** has no "kid" claim.'.format(self.short_name)
        elif header.get('alg') != 'RS256':
            error_message = (
                'Firebase ***REMOVED***0***REMOVED*** has incorrect algorithm. Expected "RS256" but got '
                '"***REMOVED***1***REMOVED***". ***REMOVED***2***REMOVED***'.format(self.short_name, header.get('alg'), verify_id_token_msg))
        elif audience != self.project_id:
            error_message = (
                'Firebase ***REMOVED***0***REMOVED*** has incorrect "aud" (audience) claim. Expected "***REMOVED***1***REMOVED***" but '
                'got "***REMOVED***2***REMOVED***". ***REMOVED***3***REMOVED*** ***REMOVED***4***REMOVED***'.format(self.short_name, self.project_id, audience,
                                            project_id_match_msg, verify_id_token_msg))
        elif issuer != expected_issuer:
            error_message = (
                'Firebase ***REMOVED***0***REMOVED*** has incorrect "iss" (issuer) claim. Expected "***REMOVED***1***REMOVED***" but '
                'got "***REMOVED***2***REMOVED***". ***REMOVED***3***REMOVED*** ***REMOVED***4***REMOVED***'.format(self.short_name, expected_issuer, issuer,
                                            project_id_match_msg, verify_id_token_msg))
        elif subject is None or not isinstance(subject, six.string_types):
            error_message = (
                'Firebase ***REMOVED***0***REMOVED*** has no "sub" (subject) claim. '
                '***REMOVED***1***REMOVED***'.format(self.short_name, verify_id_token_msg))
        elif not subject:
            error_message = (
                'Firebase ***REMOVED***0***REMOVED*** has an empty string "sub" (subject) claim. '
                '***REMOVED***1***REMOVED***'.format(self.short_name, verify_id_token_msg))
        elif len(subject) > 128:
            error_message = (
                'Firebase ***REMOVED***0***REMOVED*** has a "sub" (subject) claim longer than 128 characters. '
                '***REMOVED***1***REMOVED***'.format(self.short_name, verify_id_token_msg))
