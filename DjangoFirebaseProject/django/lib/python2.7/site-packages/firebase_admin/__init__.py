...
def initialize_app(credential=None, options=None, name=_DEFAULT_APP_NAME):
    ...
    the variable starts with ``"***REMOVED***"``, it is parsed as a JSON object. Otherwise it is treated
    ...
    if name == _DEFAULT_APP_NAME:
        ...
        raise ValueError((
            'Firebase app named "***REMOVED***0***REMOVED***" already exists. This means you called '
...
def delete_app(app):
    ...
    if not isinstance(app, App):
        raise ValueError('Illegal app argument type: "***REMOVED******REMOVED***". Argument must be of '
    ...
    if app.name == _DEFAULT_APP_NAME:
        ...
        raise ValueError(
            ('Firebase app named "***REMOVED***0***REMOVED***" is not initialized. Make sure to initialize '
...
def get_app(name=_DEFAULT_APP_NAME):
    ...
    if not isinstance(name, six.string_types):
        raise ValueError('Illegal app name argument type: "***REMOVED******REMOVED***". App name '
    ...
    if name == _DEFAULT_APP_NAME:
        ...
        raise ValueError(
            ('Firebase app named "***REMOVED***0***REMOVED***" does not exist. Make sure to initialize '
...
class _AppOptions(object):
    ...
    def __init__(self, options):
        ...
        if not isinstance(options, dict):
            raise ValueError('Illegal Firebase app options type: ***REMOVED***0***REMOVED***. Options '
    ...
    def _load_from_environment(self):
        ...
        If the value of the FIREBASE_CONFIG environment variable starts with "***REMOVED***" an attempt is made
        ...
        if config_file.startswith('***REMOVED***'):
            ...
            try:
                ...
                raise ValueError('Unable to read file ***REMOVED******REMOVED***. ***REMOVED******REMOVED***'.format(config_file, err))
        try:
            json_data = json.loads(json_str)
        except Exception as err:
            raise ValueError('JSON string "***REMOVED***0***REMOVED***" is not valid json. ***REMOVED***1***REMOVED***'.format(json_str, err))
        return ***REMOVED***k: v for k, v in json_data.items() if k in _CONFIG_VALID_KEYS***REMOVED***
...
class App(object):
    ...
    def __init__(self, name, credential, options):
        ...
        if not name or not isinstance(name, six.string_types):
            raise ValueError('Illegal Firebase app name "***REMOVED***0***REMOVED***" provided. App name must be a '
        ...
        self._services = ***REMOVED******REMOVED***
    ...
    def _lookup_project_id(cls, credential, options):
        ...
        if project_id is not None and not isinstance(project_id, six.string_types):
            raise ValueError(
                'Invalid project ID: "***REMOVED***0***REMOVED***". project ID must be a string.'.format(project_id))
        return project_id
    ...
    def _get_service(self, name, initializer):
        ...
        if not name or not isinstance(name, six.string_types):
            raise ValueError(
                'Illegal name argument: "***REMOVED***0***REMOVED***". Name must be a non-empty string.'.format(name))
        with self._lock:
            if self._services is None:
                raise ValueError(
                    'Service requested from deleted Firebase App: "***REMOVED***0***REMOVED***".'.format(self._name))
