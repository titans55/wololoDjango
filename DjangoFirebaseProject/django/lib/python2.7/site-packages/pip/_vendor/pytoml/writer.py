...
def _format_list(v):
    return '[***REMOVED***0***REMOVED***]'.format(', '.join(_format_value(obj) for obj in v))
...
def _format_value(v):
    ...
    if isinstance(v, float):
        if math.isnan(v) or math.isinf(v):
            raise ValueError("***REMOVED***0***REMOVED*** is not a valid TOML value".format(v))
        else:
            return repr(v)
        ...
        if offs == 0:
            ...
            if offs > 0:
                ...
                suffix = '-'
                offs = -offs
            suffix = '***REMOVED***0***REMOVED******REMOVED***1:.02***REMOVED******REMOVED***2:.02***REMOVED***'.format(suffix, offs // 60, offs % 60)
    ...
    while tables:
        ...
        for k in table_keys:
            ...
            if isinstance(v, dict):
                ...
                fout.write(
                    '#***REMOVED******REMOVED*** = null  # To use: uncomment and replace null with value\n'.format(_escape_id(k)))
                has_kv = True
            else:
                fout.write('***REMOVED***0***REMOVED*** = ***REMOVED***1***REMOVED***\n'.format(_escape_id(k), _format_value(v)))
                has_kv = True
