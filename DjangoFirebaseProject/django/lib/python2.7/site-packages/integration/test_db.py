...
class TestReadOperations(object):
    ...
    def test_get_shallow(self, testref):
        ...
        assert isinstance(value, dict)
        assert value == ***REMOVED***'dinosaurs': True, 'scores': True***REMOVED***
...
class TestWriteOperations(object):
    ...
    def test_push_with_value(self, testref):
        python = testref.parent
        value = ***REMOVED***'name' : 'Luis Alvarez', 'since' : 1911***REMOVED***
    ...
    def test_set_complex_value(self, testref):
        python = testref.parent
        value = ***REMOVED***'name' : 'Mary Anning', 'since' : 1799***REMOVED***
        ref = python.child('users').push()
        ref.set(value)
        assert ref.get() == value
    ...
    def test_update_children(self, testref):
        python = testref.parent
        value = ***REMOVED***'name' : 'Robert Bakker', 'since' : 1945***REMOVED***
        ref = python.child('users').push()
        ref.update(value)
        assert ref.get() == value
    ...
    def test_update_children_with_existing_values(self, testref):
        python = testref.parent
        value = ***REMOVED***'name' : 'Edwin Colbert', 'since' : 1900, 'temp': True***REMOVED***
        ref = python.child('users').push(value)
        ref.update(***REMOVED***'since' : 1905***REMOVED***)
        value['since'] = 1905
        assert ref.get() == value
        ref.update(***REMOVED***'temp': None***REMOVED***)
        del value['temp']
        assert ref.get() == value

    def test_update_nested_children(self, testref):
        python = testref.parent
        edward = python.child('users').push(***REMOVED***'name' : 'Edward Cope', 'since' : 1800***REMOVED***)
        jack = python.child('users').push(***REMOVED***'name' : 'Jack Horner', 'since' : 1940***REMOVED***)
        delta = ***REMOVED***
            '***REMOVED***0***REMOVED***/since'.format(edward.key) : 1840,
            '***REMOVED***0***REMOVED***/since'.format(jack.key) : 1946
        ***REMOVED***
        python.child('users').update(delta)
        assert edward.get() == ***REMOVED***'name' : 'Edward Cope', 'since' : 1840***REMOVED***
        assert jack.get() == ***REMOVED***'name' : 'Jack Horner', 'since' : 1946***REMOVED***

    def test_set_if_unchanged(self, testref):
        python = testref.parent
        push_data = ***REMOVED***'name' : 'Edward Cope', 'since' : 1800***REMOVED***
        edward = python.child('users').push(push_data)

        update_data = ***REMOVED***'name' : 'Jack Horner', 'since' : 1940***REMOVED***
    ...
    def test_transaction(self, testref):
        ...
        def transaction_update(snapshot):
            ...
            snapshot['since'] = 1804
            return snapshot
        ref = python.child('users').push(***REMOVED***'name' : 'Richard'***REMOVED***)
        new_value = ref.transaction(transaction_update)
        expected = ***REMOVED***'name': 'Richard Owen', 'since': 1804***REMOVED***
...
def override_app(request, update_rules):
    ...
    ops = ***REMOVED***
        'databaseURL' : 'https://***REMOVED***0***REMOVED***.firebaseio.com'.format(project_id),
        'databaseAuthVariableOverride' : ***REMOVED***'uid' : 'user1'***REMOVED***
    ***REMOVED***
