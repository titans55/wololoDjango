...
def _sign_in(custom_token, api_key):
    body = ***REMOVED***'token' : custom_token.decode(), 'returnSecureToken' : True***REMOVED***
    params = ***REMOVED***'key' : api_key***REMOVED***
...
def _sign_in_with_password(email, password, api_key):
    body = ***REMOVED***'email': email, 'password': password***REMOVED***
    params = ***REMOVED***'key' : api_key***REMOVED***
...
def _random_id():
    ...
    email = 'test***REMOVED***0***REMOVED***@example.***REMOVED***1***REMOVED***.com'.format(random_id[:12], random_id[12:])
...
def test_custom_token_without_service_account(api_key):
    ...
    custom_app = firebase_admin.initialize_app(cred, ***REMOVED***
...
def test_custom_token_with_claims(api_key):
    dev_claims = ***REMOVED***'premium' : True, 'subscription' : 'silver'***REMOVED***
...
def test_set_custom_user_claims(new_user, api_key):
    claims = ***REMOVED***'admin' : True, 'package' : 'gold'***REMOVED***
...
def test_update_custom_user_claims(new_user):
    ...
    claims = ***REMOVED***'admin' : False, 'subscription' : 'guest'***REMOVED***
    auth.set_custom_user_claims(new_user.uid, claims)
    user = auth.get_user(new_user.uid)
    assert user.custom_claims == claims
