                ...
                while True:
                    ...
                    if state.request is None:
                        if state.client is _CLOSED:
                            details = '"***REMOVED******REMOVED***" requires exactly one request message.'.format(
    ...
    try:
        ...
        with state.condition:
            if exception is state.abortion:
                ...
                details = 'Exception calling application: ***REMOVED******REMOVED***'.format(exception)
    ...
    try:
        ...
        with state.condition:
            if exception is state.abortion:
                ...
                details = 'Exception iterating responses: ***REMOVED******REMOVED***'.format(exception)
                _LOGGER.exception(details)
                _abort(state, rpc_event.call, cygrpc.StatusCode.unknown,
                       _common.encode(details))
        return None, False
...
def _handle_call(rpc_event, generic_handlers, interceptor_pipeline, thread_pool,
    ...
    if rpc_event.call_details.method is not None:
        try:
            ...
            details = 'Exception servicing handler: ***REMOVED******REMOVED***'.format(exception)
            _LOGGER.exception(details)
    ...
    for generic_rpc_handler in generic_rpc_handlers:
        ...
        if service_attribute is None:
            raise AttributeError(
                '"***REMOVED******REMOVED***" must conform to grpc.GenericRpcHandler type but does '
