...
def _get_content_type(url, session):
    ...
    if scheme not in ***REMOVED***'http', 'https'***REMOVED***:
...
class PackageFinder(object):
    ...
    def get_formatted_locations(self):
        ...
        if self.index_urls and self.index_urls != [PyPI.simple_url]:
            lines.append(
                "Looking in indexes: ***REMOVED******REMOVED***".format(", ".join(
                    remove_auth_from_url(url) for url in self.index_urls))
            )
        if self.find_links:
            lines.append(
                "Looking in links: ***REMOVED******REMOVED***".format(", ".join(self.find_links))
            )
        return "\n".join(lines)
    ...
    def find_requirement(self, req, upgrade):
        ...
        if installed_version is None and best_candidate is None:
            logger.critical(
                ...
                ', '.join(
                    sorted(
                        ***REMOVED***str(c.version) for c in all_candidates***REMOVED***,
                        key=parse_version,
                    )
                )
            )
