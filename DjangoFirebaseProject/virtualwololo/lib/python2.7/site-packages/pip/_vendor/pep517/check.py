...
def check_build_sdist(hooks):
    with BuildEnvironment() as env:
        ...
        try:
            reqs = hooks.get_requires_for_build_sdist(***REMOVED******REMOVED***)
        ...
        try:
            try:
                filename = hooks.build_sdist(td, ***REMOVED******REMOVED***)
...
def check_build_wheel(hooks):
    with BuildEnvironment() as env:
        ...
        try:
            reqs = hooks.get_requires_for_build_wheel(***REMOVED******REMOVED***)
            log.info('Got build requires: %s', reqs)
        except:
            log.error('Failure in get_requires_for_build_sdist', exc_info=True)
            return False
        ...
        try:
            try:
                filename = hooks.build_wheel(td, ***REMOVED******REMOVED***)
