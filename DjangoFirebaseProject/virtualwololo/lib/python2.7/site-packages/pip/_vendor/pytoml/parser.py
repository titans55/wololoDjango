...
def loads(s, filename='<string>', translate=lambda t, x, v: v, object_pairs_hook=dict):
    ...
    for kind, value, pos in ast:
        if kind == 'kv':
            k, v = value
            if k in scope:
                error('duplicate_keys. Key "***REMOVED***0***REMOVED***" was used more than once.'.format(k))
...
_escapes = ***REMOVED*** 'b': '\b', 'n': '\n', 'r': '\r', 't': '\t', '"': '"', '\'': '\'',
    '\\': '\\', '/': '/', 'f': '\f' ***REMOVED***

_basicstr_re = re.compile(r'[^"\\\000-\037]*')
_short_uni_re = re.compile(r'u([0-9a-fA-F]***REMOVED***4***REMOVED***)')
_long_uni_re = re.compile(r'U([0-9a-fA-F]***REMOVED***8***REMOVED***)')
...
_datetime_re = re.compile(r'(\d***REMOVED***4***REMOVED***)-(\d***REMOVED***2***REMOVED***)-(\d***REMOVED***2***REMOVED***)T(\d***REMOVED***2***REMOVED***):(\d***REMOVED***2***REMOVED***):(\d***REMOVED***2***REMOVED***)(\.\d+)?(?:Z|([+-]\d***REMOVED***2***REMOVED***):(\d***REMOVED***2***REMOVED***))')
...
def _p_value(s, object_pairs_hook):
    ...
    if s.consume('***REMOVED***'):
        _p_ws(s)
        items = object_pairs_hook()
        if not s.consume('***REMOVED***'):
            k = _p_key(s)
            _p_ws(s)
            s.expect('=')
            _p_ws(s)
...
class _TimeZone(datetime.tzinfo):
    ...
    def tzname(self, dt):
        ...
        h = m // 60
        m = m - h * 60
        return '***REMOVED******REMOVED******REMOVED***:.02***REMOVED******REMOVED***:.02***REMOVED***'.format(res, h, m)
